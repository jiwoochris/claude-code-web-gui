...
> **버전** 0.5 · **작성일** 2026-04-24 · **배포 범위** 사내망 전용
>
> **변경 이력**
...
- **완전 읽기 전용**. 파일 수정·생성·삭제 API는 존재하지 않는다. Docker 배포 시 볼륨도 `:ro`로 마운트해 이중 안전장치를 둔다.
...
- **읽기 전용 보장**: `/api/fs/*` 라우트에 쓰기 메서드(POST/PUT/DELETE/PATCH) 구현 없음. Docker에선 `:ro` 볼륨으로 이중 안전.
...
services:
  ui:
    volumes:
      - /Users/sigmine/projects:/workspace:ro    # 또는 호스트 경로에 맞게
```
